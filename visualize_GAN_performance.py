...
class GAN_grid_video(GAN_visualization):
    def set_vid_params(self, ft = 20, st = 1, iz = 1, fps = 24):
        ...
        self.n_frames = int(np.rint(self.duration * self.fps))
        self.random_state = np.random.RandomState(self.random_seed)
    def generate_all_latents(self):
        shape = [self.n_frames, np.prod([self.n_img_x, self.n_img_y])] + self.Gs.input_shape[1:]
    ...
    def generate_frame(self, t):
        frame_idx = int(np.clip(np.round(t * self.fps), 0, self.n_frames - 1))
    ...
    def generate_video(self, out_path):
        video = moviepy.editor.VideoClip(self.generate_frame, duration = self.duration)
        video.write_videofile(out_path, fps = self.fps)

class GAN_progress:
    def __init__(self, n_img_y, n_img_x, trunc, stylegan_path):
        sys.path.append(stylegan_path)
        global dnnlib, tflib
        import dnnlib
        import dnnlib.tflib as tflib
    ...
    def set_vid_params(self, ft = 20, st = 1, iz = 1, fps = 24):
        ...
        self.n_frames = int(np.rint(self.duration * self.fps))
        self.random_state = np.random.RandomState(self.random_seed)
    def generate_all_latents(self, example_path):
        Gs = self.get_model(example_path)
        shape = [self.n_frames, np.prod([self.n_img_x, self.n_img_y])] + Gs.input_shape[1:]
        zs = self.random_state.randn(*shape).astype(np.float32)
        zs = scipy.ndimage.gaussian_filter(zs, [self.smooth_time * self.fps] + [0] * len(Gs.input_shape), mode = "wrap")
        zs /= np.sqrt(np.mean(np.square(zs)))
        self.zs = zs
    def get_image_grid(self, imgs):
        _, img_h, img_w, ch = imgs.shape
        ...
        return grid
    def generate_frame(self, t):
        if not self.stop_indexing:
            idx = ceil(t / (floor(self.n_frames / self.n_models) * 1 / self.fps))
        if idx > self.idx:
            self.idx = idx
            if self.idx > self.n_models - 1:
                self.idx = self.n_models - 1
                idx = self.idx
                self.stop_indexing = True
            path = self.paths[self.idx]
            self.Gs = self.get_model(path)          
        frame_idx = int(np.clip(np.round(t * self.fps), 0, self.n_frames - 1))
        latent_vecs = self.zs[frame_idx]
        fmt = dict(func = tflib.convert_images_to_uint8, nchw_to_nhwc = True)
        imgs = self.Gs.run(latent_vecs, None, truncation_psi = self.trunc, randomize_noise = False, output_transform = fmt, minibatch_size = 16)
        grid = self.get_image_grid(imgs)
        return grid
    def generate_video(self, pkl_paths, out_path):
        self.paths = []
        for file in os.listdir(pkl_paths):
            file_str = str(pkl_paths + file)
            if ".pkl" in file_str:
                self.paths.append(file_str)
        self.paths.sort()
        self.idx = -1
        self.n_models = len(self.paths)
